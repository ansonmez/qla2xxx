	...
	case SRB_ELS_CMD_HST_NOLOGIN:
		type = "els";
		{
			...
			ql_dbg(ql_dbg_user, vha, 0x700f,
			     "%s %s. portid=%02x%02x%02x status %x xchg %x bsg ptr %px\n",
			...
			if (! (le16_to_cpu(els->control_flags) & ECF_PAYLOAD_DESCR_MASK)) {
				if (sp->remap.remapped) {
					...
					ql_print_bsg_sglist(ql_dbg_user+ql_dbg_verbose, vha, 0x700f,
					   "SG bsg->reply",
			...
			if (logit) {
				if (sp->remap.remapped &&
					...
					ql_dbg(ql_dbg_user, vha, 0x503f,
					    ...
					    "total_byte=0x%x %02x%02x%02x -> %02x%02x%02x\n",
					    type, sp->handle, comp_status, fw_status[1],
					    fw_status[2], le32_to_cpu(((struct els_sts_entry_24xx *)
						pkt)->total_byte_count),
...
qla27xx_status_cont_type_1(scsi_qla_host_t *vha, sts_cont_entry_t *pkt)
{
	...
	if (ha->flags.flogi_acc_pl_in_cont_iocb) {
		if (ha->flogi_acc_curr_offset == 0) {
			...
			page_code = (uint8_t )(*((uint16_t *)pkt + 10) & 0xff);
			page_length = *((uint16_t *)pkt + 11);

			if ((page_code == 0xf0) && (page_length > 0)) {
				memcpy(vendor_id_data,
				   (uint8_t *)((uint16_t *)pkt +12), 8);
				if (memcmp(vendor_id_data, "CISCO", 5) == 0) {
					if (ha->flogi_acc_cl3_sp_options & 1) {
						ha->flags.conn_fabric_cisco = 1;
					} else {
						ql_dbg(ql_log_warn, vha, 0x5075,
						    "Cisco Fabric, Priority"
						    " not supported\n");
					}
				 ...
				 int length)
{
	...
	if (be32_to_cpu(fpin_desc->link_integrity.port_name_count) > 0) {
		for (i = 0;
		    i < be32_to_cpu(fpin_desc->link_integrity.port_name_count);
		    i++) {
			if (offsetof(struct fpin_descriptor,
			    link_integrity.port_name_list[i]) >
			    length - 8) {
				ql_log(ql_log_warn, fcport->vha, 0x5098,
				    "SCM: Port list[%d] exceeds payload size, len %d\n",
				    i, length);
				break;
			}
					...
					int length)
{
	...
	if (be32_to_cpu(fpin_desc->peer_congestion.port_name_count) > 0) {
		for (i = 0;
		    ...
		    i++) {
			if (offsetof(struct fpin_descriptor,
			    peer_congestion.port_name_list[i]) >
			    length - 8) {
				ql_log(ql_log_warn, fcport->vha, 0x5098,
				    "SCM: Port list[%d] exceeds payload size, len %d\n",
				    i, length);
...
static void
qla_scm_clear_pervious_event(struct scsi_qla_host *vha)
{
	fc_port_t *fcport = NULL;
	struct qla_hw_data *ha = vha->hw;
...
qla27xx_process_purex_fpin(struct scsi_qla_host *vha, struct purex_item *item)
{
	...
	qla_scm_clear_pervious_event(vha);
	...
	if (fpin_desc_len != be32_to_cpu(fpin_desc->descriptor_length) + 8) {
		ql_log(ql_log_warn, vha, 0x5099,
		    "Length mis-match, desc len=%d, actual len=%d\n",
	...
	while (fpin_offset <= fpin_desc_len) {
		...
		if (fpin_desc_len < (fpin_offset + FPIN_DESCRIPTOR_HEADER_SIZE))
			return;
	...
	struct rsp_que *rsp, response_t *pkt)
{
	...
	rsp_q_in = rsp->qpair->use_shadow_reg ? *rsp->in_ptr :
	    RD_REG_DWORD_RELAXED(rsp->rsp_q_in);
	...
	ql_dbg(ql_dbg_init + ql_dbg_verbose, vha, 0x5091,
	    "%s - ring %px pkt %px end pkt %px entry count %#x rsp_q_in %d "
	    "rc %d\n",
