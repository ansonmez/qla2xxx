    ...
    struct nvmefc_fcp_req *fd)
{
	...
	if (IS_SCM_CAPABLE(ha)) {
		...
		qla2xxx_update_sfc_ios(ha, fcport, fd->payload_length);
	}
	...
	if (rval != QLA_SUCCESS) {
		...
		wake_up(&sp->nvme_ls_waitq);
		qla2xxx_atomic_sub(&ha->sfc.scmr_bytes,
				   fd->payload_length);
		qla2xxx_atomic_sub(&fcport->sfc.scmr_bytes,
				   fd->payload_length);
...
void qla_nvme_abort_set_option(
	struct abort_entry_24xx *abt, srb_t *orig_sp)
{
	struct qla_hw_data *ha;
		...
		struct abort_entry_24xx *abt, srb_t *orig_sp)
{
	...
	case CS_COMPLETE:
		ql_dbg (ql_dbg_async + ql_dbg_verbose, vha, 0xf09f,
		    "IOCB request is completed sucessfully \
		    comp_status=%x\n",comp_status);
		break;
