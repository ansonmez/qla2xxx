...
int qla2x00_sys_ld_info(bsg_job_t *bsg_job)
{
	...
	/* 
 	 ...
 	 */
	if ((rval != QLA_SUCCESS) ||
		(ld_feature != MPI_LOCKDOWN_FUNCTIONAL)) {
		ql_log(ql_log_warn, vha, 0x0067,
		    "Unable to read fw lockdown features or not supported \
			rval=%d ld_feature=%u\n",rval,(u16)ld_feature);
		...
		bsg_reply->reply_payload_rcv_len =
			...
			bsg_job->reply_payload.sg_cnt, i, sizeof(*i));
