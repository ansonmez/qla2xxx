			   ...
			   char *buf, loff_t off, size_t count)
{
	...
	if (IS_P3P_TYPE(ha)) {
		...
		rval = memory_read_from_buffer(buf, count, &off,
				ha->mpi_fw_dump, ha->mpi_fw_dump_len);
...
static DEVICE_ATTR(nvme_connect_str, S_IRUGO,
		   qla27xx_nvme_connect_str_show, NULL);

static ssize_t
qla2x00_dport_diagnostics_show(struct device *dev, struct device_attribute *attr,
    char *buf)
{
	scsi_qla_host_t *vha = shost_priv(class_to_shost(dev));

	if (!IS_QLA83XX(vha->hw) && !IS_QLA27XX(vha->hw) && !IS_QLA28XX(vha->hw))
		return scnprintf(buf, PAGE_SIZE, "\n");
...
static DEVICE_ATTR(dport_diagnostics, S_IRUGO,
	   qla2x00_dport_diagnostics_show, NULL);

static ssize_t
qla2xxx_scmr_stat_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	scsi_qla_host_t *vha = shost_priv(class_to_shost(dev));
	struct qla_hw_data *ha = vha->hw;
...
static ssize_t
qla2xxx_scmr_stat_set(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int val;
	scsi_qla_host_t *vha = shost_priv(class_to_shost(dev));
...
static ssize_t
qla2xxx_scmr_signal_set(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int val;
	scsi_qla_host_t *vha = shost_priv(class_to_shost(dev));
	...
	ql_log(ql_log_warn, vha, 0x0301,
			"USER: Sending SCMR signal %d.\n", val);
	atomic_set(&vha->hw->sfc.scmr_congn_signal, val);
...
static DEVICE_ATTR(beacon_config, S_IRUGO | S_IWUSR, qla2x00_beacon_config_show,
...
static DEVICE_ATTR(edif_doorbell,  S_IRUGO, qla2x00_edif_doorbellr, NULL);
static DEVICE_ATTR(scmr_stat, 0600, qla2xxx_scmr_stat_show,
		   qla2xxx_scmr_stat_set);
static DEVICE_ATTR(scmr_signal, 0200, NULL, qla2xxx_scmr_signal_set);
