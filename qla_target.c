...
static void qla2x00_async_nack_sp_done(srb_t *sp, int res)
{
	...
	case SRB_NACK_PRLI:
		...
		if (!sp->fcport->login_succ &&
			...
			qla2x00_set_fcport_disc_state(sp->fcport, DSC_LOGIN_COMPLETE);
			sp->fcport->deleted = 0;
			sp->fcport->logout_on_delete = 1;
		}
		break;
...
void qla24xx_delete_sess_fn(struct work_struct *work)
{
	...
	if (fcport == NULL || fcport->vha == NULL || fcport->vha->hw == NULL) {
		return;
	}
...
void qlt_schedule_sess_for_deletion(struct fc_port *sess)
{
	...
	if (IS_SCM_CAPABLE(sess->vha->hw)) {
		qla2xxx_scmr_clear_congn(&sess->sfc);
		qla2xxx_scmr_clear_throttle(&sess->sfc);
	}
...
#if 0
	if ((prm->cmd->bufflen > 0) && prm->cmd->sess->edif.enable) {
		if (prm->cmd->sess->edif.tx_sa_selold)
			pkt->u.status0.edif_flags |= EF_EN_EDIF;
		else
			pkt->u.status0.edif_flags |= (EF_EN_EDIF | EF_NEW_SA);
	}
#endif
	...
	if (unlikely(status != CTIO_SUCCESS)) {
		...
		case CTIO_FAST_SPI_ERR:
			ql_dbg(ql_dbg_tgt_mgt, vha, 0xf05b,
			    "qla_target(%d): CTIO with EDIF error status 0x%x "
			    "received (state %x, se_cmd %px\n",
			    vha->vp_idx, status, cmd->state, se_cmd);
			break;
    ...
    struct imm_ntfy_from_isp *iocb)
{
	...
	if (vha->hw->flags.edif_enabled) {
		if (!(vha->e_dbell.db_flags & EDB_ACTIVE)) {
		    ...
		    !(iocb->u.isp24.flags & NOTIFY24XX_FLAGS_FCSP)) {
			ql_dbg(ql_dbg_disc, vha, 0xffff,
