...
qla2x00_mailbox_command(scsi_qla_host_t *vha, mbx_cmd_t *mcp)
{
	...
	if (ha->flags.purge_mbox || chip_reset != ha->chip_reset ||
		...
		ql_log(ql_log_warn, vha, 0xd035,
		    ...
		    ha->flags.purge_mbox, ha->flags.eeh_busy,
		    mcp->mb[0]);
		rval = QLA_ABORTED;
		goto premature_exit;
	}
...
mbx_done:
	...
	while (i && eeh_delay && (ha->pci_error_state < QLA_PCI_SLOT_RESET)) {
		...
		sure caller does not unmap dma while recovery is in progress */
		msleep(1);
		i--;
	}
	return rval;
	...
	if (IS_QLA27XX(ha) || IS_QLA28XX(ha)) {
		...
		if(ha->flags.scm_supported_a &&
			...
			if (ha->sf_init_cb) {
				...
				ha->sf_init_cb->flags |= cpu_to_le16(BIT_11);
				ha->sf_init_cb->flags |= cpu_to_le16(BIT_10);
				if (ql2xcontrol_edc_rdf) {
					ha->sf_init_cb->flags |= cpu_to_le16(BIT_7);
					ql_log(ql_log_info, vha, 0xffff,
...
qla25xx_set_els_cmds_supported(scsi_qla_host_t *vha)
{
	...
	if (ha->flags.scm_supported_f) {
		...
		active_cnt++;
		cmd_opcode[active_cnt] = ELS_COMMAND_PUN;
		active_cnt++;
	}
	if (ha->flags.edif_enabled) {
...
int
qla2xxx_set_scm_params(fc_port_t *fcport,
		       bool congested)
{
	int rval;
	mbx_cmd_t mc;
	...
	mcp->mb[9] = vha->vp_idx;
	mcp->out_mb = MBX_3|MBX_1|MBX_0;
	mcp->in_mb = MBX_9|MBX_3|MBX_2|MBX_1|MBX_0;
	mcp->tov = 30;
	mcp->flags = 0;
	rval = qla2x00_mailbox_command(vha, mcp);
