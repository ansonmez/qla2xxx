...
#define QLA_SCMRF_FAST_TGT		7
	...
	atomic_t scmr_bytes;
	atomic_t scmr_bytes_per_period;
...
// edif hash element
typedef struct edif_list_entry {
...
typedef struct sa_index_fp_entry {
	uint16_t sa_index;
	struct list_head next;
}sa_index_fp_entry_t;


typedef struct sa_index_pair {
	uint16_t sa_index;
	uint32_t spi;
} sa_index_pair_t;
...
typedef struct edif_sa_index_entry {
	sa_index_pair_t sa_pair[2];
	fc_port_t *fcport;
	uint16_t handle;
	struct list_head next;
} edif_sa_index_entry_t;
